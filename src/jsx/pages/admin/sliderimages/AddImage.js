import { GetAllTenants, SaveSliderImages, UpdateSliderImages } from "../../../../services/CommonService";
import { useEffect, useState } from "react";
const AddImage=({dataprops})=>{
    const fields = {
        base64:'',
        tenantid: '',
        status: '',
     ...
     const submitForm = async () => {
        console.log('formfields,' ,formfields);
         ...
         if (isValid) {
             if (!formfields.id) {
                 //save
                 const Response = await SaveSliderImages(formfields);
     
             } else {
                 //update
                 const Response = await UpdateSliderImages(formfields);
             }
